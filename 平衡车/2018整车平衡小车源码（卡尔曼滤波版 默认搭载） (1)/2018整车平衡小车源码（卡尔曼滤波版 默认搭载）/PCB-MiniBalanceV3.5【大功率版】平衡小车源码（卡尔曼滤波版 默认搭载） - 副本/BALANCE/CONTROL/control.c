...
//	float Bias,kp=340, kd=0.9;
//	 int balance;
//	 Bias=Angle-ZHONGZHI;       //===求出平衡的角度中值 和机械相关
//	 balance=kp*Bias+Gyro*kd;   //===计算平衡控制的电机PWM  PD控制   kp是P系数 kd是D系数 
//	 return balance;
	
		float Bias,kp=340, kd=1;
	 int balance;
	
	//		if(1==Flag_Qian)    Bias=Angle-ZHONGZHI+1;         //===前进标志位置1 
//		else if(1==Flag_Hou)	Bias=Angle-ZHONGZHI-1;       //===后退标志位置1
//	else 
	 Bias=Angle-ZHONGZHI;       //===求出平衡的角度中值 和机械相关
...
{  
	...
	//  float kp=80,ki=0.4;
  float kp=80 ,ki=0.4 ;	
	int Distance_set=300;
  //=============遥控前进后退部分=======================// 
	  if(Bi_zhang==1&&Flag_sudu==1)  Target_Velocity=80;                 //如果进入避障模式,自动进入低速模式
  //  else 	                         Target_Velocity=110;                 
	  ...
	  if(Bi_zhang==1&&Distance<Distance_set&&Flag_Left!=1&&Flag_Right!=1)        //避障标志位置1且非遥控转弯的时候，进入避障模式
...
int turn(int encoder_left,int encoder_right,float gyro)//转向控制
{
	 static float Turn_Target,Turn,Encoder_temp,Turn_Convert=0.9,Turn_Count;
	  float Turn_Amplitude=80/Flag_sudu,Kp=30,Kd=0;     
	  //=============遥控左右旋转部分=======================//
...
{
	    u8 temp;
			if(angle<-40||angle>40||1==Flag_Stop||voltage<1000)//电池电压低于10.0V关闭电机
			{	                                                 //===倾角大于40度关闭电机
